...
const { format, captureAnchor, vw } = require('../components/utils')
...
function view (state, emit) {
  ...
  if (!cooperative.needUpdate) {
    if (typeof cooperative.hasLaundryRoom !== 'undefined') {
      ...
      details[key] = html`<span class="u-hiddenVisually">${__('Yes')}</span>`
    }
    ...
    <div class="App-container">
      <div class="App-part App-part--secondary App-part--last u-marginBm">
        <div class="Sheet Sheet--conditional Sheet--md Sheet--lg">
          ...
          <!-- Cooperative details -->
          ${state.cache(Definition, `${cooperative._id}-details`).render(details, 3)}

          <!-- Cooperative initatives -->
          ${INITIATIVES.find(hasInitiative(cooperative)) ? html`
            <div class="u-marginTm">
              <h2 class="Display Display--4 u-marginBs u-textItalic u-flex u-flexJustifyBetween u-flexAlignItemsBaseline">
                <span>${__('Energy initiatives')}</span> <button class="Link u-textB u-textNormal">${__('Show')}</a>
              </h2>
              <ul>
