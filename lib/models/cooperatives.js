...
CooperativeSchema.pre('save', async function (next) {
  ...
  if (this.hasRegistered && this.isModified('hasRegistered')) {
    // Email admin on new registration
    await Prismic.api(process.env.PRISMIC_API).then((api) => {
      return api.getByID('WrSwoCYAACsAjFpd').then(async (doc) => {
        const serialize = email.serialize({
          date: moment().format('HH:MM, YYYY-MM-DD'),
          user: await User.get(this.editors[0]),
          cooperative: this
        })

        return email.send({
          subject: asText(doc.data.subject),
          to: process.env.EMAIL_ADDRESS,
          html: dedent`
            <body>
              ${asElement(doc.data.body, email.resolve, serialize).join('')}
            </body>
          `
        })
      })
