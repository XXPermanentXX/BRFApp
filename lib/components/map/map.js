...
module.exports = component({
  ...
  update(element, args, prev) {
    ...
    if (!this.map && this.hasLoaded && center) {
  ...
  load(element, cooperatives, center) {
    ...
    load([
      ...
      if (center && !this.isInitialized) {
        this.init(element, cooperatives, center);
      }
    });
  },

  init(element, cooperatives, center) {
  ...
  render() {
    return html`
      <div class="Map u-sizeFill is-loading" id="map-canvas">
        <div class="u-colorSky u-sizeFull" id="map-loader">${ loader() }</div>
      </div>
    `;
  },
