              ...
              ${this.search ? html`
                <ul class="List">
                  ${results.length ? results.slice(0, 6).map(item => html`
                    <li class="List-item">
                      <button class="Button Button--link u-block u-sizeFull u-paddingAb" name="_id" value="${item._id}" onclick=${setProp}>
                        ${item.name}
                      </button>
                    </li>
                  `) : html`
        ...
        <div role="tabpanel" id="${this.id}-filter">
          ${this.panel === 'filter' ? html`
            <fieldset class="u-nbfc">
              ...
              <p class="Form-label u-colorDark u-marginAs">${__('Pick one or more energy initiatives below.')}</p>
              ${[
