...
module.exports = class Mapbox extends Component {
  ...
  update (cooperatives, center) {
    if (!this.map && this.mapboxgl && center) {
      ...
      const changed = cooperatives.reduce((hasChanged, cooperative, index) => {
        return hasChanged || cooperative._id !== this.cooperatives[index]
      }, false)
      ...
      if (!this.map.getSource('cooperatives')) {
        ...
        if (changed) {
          ...
          this.map.fitBounds(bounds, { padding: this.element.offsetWidth * 0.1 })
        }
      }
    }
