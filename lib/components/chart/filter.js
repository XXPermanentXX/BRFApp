      ...
      <form class="Form u-flex u-flexCol u-flexJustifyEnd u-md-flexRow u-lg-flexRow u-md-flexAlignItemsBaseline u-lg-flexAlignItemsBaseline u-paddingLb">
        ...
        <div class="u-md-flexOrderLast u-lg-flexOrderLast u-marginRb">
          <div class="Form-switchGroup u-marginBs u-sizeFull">
            <label class="Form-switch u-flexGrow1">
              <input class="u-hiddenVisually" type="radio" name="consumptions:granularity" value="month" onchange=${onchange} checked=${granularity === 'month'} disabled=${this.disabled}/>
              <span class="Form-label">${__('Monthly')}</span>
            </label>
            <label class="Form-switch u-flexGrow1">
              <input class="u-hiddenVisually" type="radio" name="consumptions:granularity" value="year" onchange=${onchange} checked=${granularity === 'year'} disabled=${this.disabled} />
          ...
          <label class="u-marginBs u-sizeFull">
            <span class="Form-pill Form-pill--single ${type !== 'heat' || this.disabled ? 'is-disabled' : ''}">
              ${__('Normalize')}
              <input type="checkbox" name="consumptions:normalized" onchange=${onchange} checked=${normalized} disabled=${type !== 'heat' || this.disabled} class="Form-toggle">
        ...
        <div class="u-marginRb">
          <label class="u-flex u-posRelative u-marginBs">
            <select class="u-overlay" name="consumptions:type" onchange=${onchange} disabled=${this.disabled}>
              ${typeOptions.map(({ value, label, isSelected, disabled }) => html`
                <option disabled=${this.disabled} value="${value}" selected=${isSelected}>
                  ${label}
                </option>
              `)}
            </select>
          ...
          <label class="u-flex u-posRelative">
            <select class="u-overlay" name="consumptions:compare" disabled=${this.disabled} onchange=${oncompare}>
