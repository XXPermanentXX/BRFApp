  ...
  return component({
    ...
    update(element, args, prev) {
      if (this.chart) {
        ...
        if (!isLoading && shouldUpdate(args, prev)) {
        ...
        // Overwrite init arguments with latest
        this.next = [element].concat(args);
      }
    ...
    load(...args) {
      if (this.hasLoaded) { return this.init(...args); }
      this.hasLoaded = true;

      // Save initialization arguments on self for possible overwrite
      this.next = args;
    ...
    init(element, granularity, actions, data, isLoading) {
      ...
      this.chart = this.Highcharts.chart(element, config, chart => chart.reflow());
