...
module.exports = class Search extends Nanocomponent {
  constructor () {
    super('compare:search')
    this.text = ''
  }
  ...
  createElement (cooperative, state, onselect) {
    this.cooperative = cooperative._id

    const { cooperatives, consumptions: { type } } = state
    ...
    const results = this.text ? cooperatives.filter(item => {
      ...
      if (!item.meters.find(meter => meter.type === type)) return false
        ...
        ${this.text ? html`
          <ul class="List">
            ${results.length ? results.slice(0, 6).map(item => html`
              <li class="List-item">
                <button class="Button Button--link u-block u-sizeFull u-paddingAb" type="submit" name="consumptions:compare" value="cooperative:${item._id}" onclick=${onselect}>
                  ${item.name}
                </button>
              </li>
            `) : html`
