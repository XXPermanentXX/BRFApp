...
module.exports = class PageHead extends Component {
  ...
  createElement () {
    ...
    const pages = {
      ...
      forum: (state) => {
        var notificationString = state.user.isAuthenticated
          ? ' (' + state.user.notificationsAmount + ')'
          : '+'
        return {
          href: process.env.FORUM_URL + (state.user.isAuthenticated
            ? '/auth/brf?brfauth=' + state.user.forumAuthenticationToken
            : '/authmetryifneeded'),
          title: __('Forum') + notificationString + state.user.status
        }
      }
    }
