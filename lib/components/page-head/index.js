...
module.exports = component({
  ...
  render (state, emit) {
    ...
    const signin = () => {
      ...
      return popup({
        title: doc.getStructuredText('sign-in.title').asText(),
        body: doc.getStructuredText('sign-in.body').asElement(),
        links: [{
    ...
    const signup = () => {
      ...
      return popup({
        title: doc.getStructuredText('registration.disclaimer_title').asText(),
        body: doc.getStructuredText('registration.disclaimer_body').asElement(),
        links: [{
        ...
        <nav class="PageHead-navigation">
          ...
          ${user.isAuthenticated ? html`
            <div id="page-menu-lg" class="PageHead-menu u-hidden u-md-block u-lg-block ${this.isExpanded ? 'is-open' : ''}">
              ${menu(Object.values(pick(pages, 'home', 'auth', 'tracking')).map(page => page(state, emit)).filter(Boolean))}
            </div>
          ` : null}
