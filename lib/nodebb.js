...
async function authenticate (data) {
  ...
  const endpoint = url.resolve(process.env.FORUM_URL, 'api/brfauth/uid')
...
async function touch (token) {
  ...
  const endpoint = url.resolve(process.env.FORUM_URL, 'api/brftouch')
  const res = await got(endpoint, opts)
  return res.body
}
...
async function register (token) {
  const opts = { json: true, method: 'POST', body: { token } }
  const endpoint = url.resolve(process.env.FORUM_URL, 'api/v2/users')
  const res = await got(endpoint, opts)
  return res.body
}
