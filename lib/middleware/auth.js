...
exports.nodebb = function () {
  return function (req, res, next) {
    if (req.isAuthenticated()) {
      if (!req.user) {
        console.log('Error: Authed but no user')
        return next()
      }
      ...
      var jwtOptions = { expiresIn: '10h' }
      var webToken = jwt.sign({msg: encryptedMessage}, process.env.BRFENERGI_SESSION_SECRET, jwtOptions)
      req.user.nodebbAuthJWT = webToken
      console.log('user: ' + req.user.nodebbAuthJWT)
    }
    next()
  }
}
