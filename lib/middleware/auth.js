  ...
  passport.use('metry', new MetryOAuth({
    ...
    Users.model.findOne({ metryId: profile._id }, async (err, user) => {
      ...
      try {
        ...
        /**
         * Performing a request to the brf auth endpoint creates an account if it does not exist.
         * We actually don't care about the response.
         */

        const authPath = process.env.FORUM_URL + '/auth/brf?brfauth=' + nodebb.authenticationToken(user)
        try {
          await got(authPath)
        } catch (e) {
          console.log('Error while touching user forum account: ' + e);
        }
