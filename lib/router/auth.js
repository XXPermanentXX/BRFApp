...
router.post('/', body(), async function (ctx, next) {
  ...
  if (!ctx.isAuthenticated()) {
    ...
    try {
      var profile = await nodebb.authenticate({ username: email, password })
    } catch (err) {
...
router.post('/sign-up', body(), async function (ctx, next) {
  ...
  if (!ctx.isAuthenticated()) {
    ...
    try {
      ...
      var profile = await nodebb.register(token)
    } catch (err) {
    ...
    let user = await Users.model.findOne({ uid: profile.payload.uid })
