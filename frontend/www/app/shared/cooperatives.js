angular.module('civis.youpower')
  ...
  .factory('Cooperatives', function ($resource, $http, Config) {
    var result = $resource(Config.host + '/api/cooperative/:id', {
      id: '@_id'
    }, {
      update: {
        method: 'PUT'
    ...
    result.prototype.getEnergyData = function (type, granularity, from, to, normalized) {
      return $http.get(Config.host + '/api/cooperative/' + this._id + '/consumption/' +
        type + '/' +
        granularity + '?from=' +
        from + (to ? ('&to=' + to) : '') + '&normalized=' +
        normalized, {
          cached: true,
        });
    };

    result.prototype.getEnergyDataFromCooperative = function (type, granularity, period, id, normalized) {
    ...
    result.prototype.getEnergyClass = function () {
      ...
      if (performance && performance <= energyClassReqValue * 0.5) {
        return 'A';
      } else if (performance > energyClassReqValue * 0.5 && performance <= energyClassReqValue * 0.75) {
        return 'B';
      } else if (performance > energyClassReqValue * 0.75 && performance <= energyClassReqValue * 1.0) {
        return 'C';
      } else if (performance > energyClassReqValue * 1.0 && performance <= energyClassReqValue * 1.35) {
        return 'D';
      } else if (performance > energyClassReqValue * 1.35 && performance <= energyClassReqValue * 1.8) {
        return 'E';
      } else if (performance > energyClassReqValue * 1.8 && performance <= energyClassReqValue * 2.35) {
        return 'F';
      } else if (performance > energyClassReqValue * 2.35) {
        return 'G';
      } else {
        return 'unknown';
      }
